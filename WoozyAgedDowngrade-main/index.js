...
app.post("/api/start-workflow", async (req, res) => {
  try {
    const n8nWebhookUrl =
      "https://rightpick.app.n8n.cloud/webhook-test/497742a8-de85-414d-aa5e-12b5c510e7c7";
    ...
    try {
      ...
      console.error("Failed to parse JSON (start-workflow):", e);
...
app.post("/api/resume-workflow", async (req, res) => {
  try {
    ...
    try {
      ...
      console.error("Failed to parse JSON (resume-workflow):", e);
      return res.status(500).json({
        error: true,
        message: "Invalid JSON response from n8n",
        raw: text,
